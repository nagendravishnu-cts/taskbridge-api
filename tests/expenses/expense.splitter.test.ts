import { ExpenseSplitter } from '../../src/expenses/services/ExpenseSplitter';
import { ExpenseService } from '../../src/expenses/services/ExpenseService';

describe('ExpenseSplitter', () => {
  it('splits an expense equally across participants', () => {
    const result = ExpenseSplitter.calculateSplit({
      totalAmount: 120,
      splitType: 'equal',
  ...
  it('rejects negative total amounts', () => {
    expect(() =>
      ExpenseSplitter.calculateSplit({
        totalAmount: -10,
        splitType: 'equal',
  ...
  it('calculates net balances from expenses and payments', () => {
    const result = ExpenseSplitter.calculateNetBalances([
...
describe('ExpenseService', () => {
  ...
  beforeEach(() => {
    ...
    service = new ExpenseService(repository, auditService, logger);
  });
  ...
  it('rejects creating an expense for another organisation', async () => {
    ...
    } as any)).rejects.toThrow('Organisation mismatch');
  });
  ...
  it('requires the minimum organisation, project, and user identifiers', async () => {
    await expect(service.createExpense({
      ...
      splitType: 'equal',
      participants: ['user-1'],
    } as any)).rejects.toThrow('Organisation ID, project ID and payer user ID are required');

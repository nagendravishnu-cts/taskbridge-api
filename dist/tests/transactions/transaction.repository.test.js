...
const TransactionRepository_1 = require("../../src/transactions/repositories/TransactionRepository");
/**
...
describe('TransactionRepository', () => {
    ...
    beforeEach(() => {
        ...
        repository = new TransactionRepository_1.TransactionRepository(mockDataSource, mockLogger);
    });
    describe('create', () => {
        it('should create and save a transaction with organisation ID', async () => {
            ...
            expect(mockLogger.info).toHaveBeenCalledWith('Transaction created', expect.objectContaining({
                organisationId,
                type: 'payment',
                amount: 100.5,
            }));
        ...
        it('should include metadata when provided', async () => {
            ...
            expect(mockTypeOrmRepository.create).toHaveBeenCalledWith(expect.objectContaining({
                metadata: { reference: 'REF-123' },
            }));
        });
    });
    describe('getByUser', () => {
        it('should retrieve transactions for a user with organisation isolation', async () => {
            ...
            expect(mockLogger.debug).toHaveBeenCalledWith('Transactions retrieved for user', expect.objectContaining({
                organisationId,
                userId,
                count: 2,
            }));
        ...
        it('should return empty array for user with no transactions', async () => {
            ...
            expect(mockLogger.debug).toHaveBeenCalledWith('Transactions retrieved for user', expect.objectContaining({ count: 0 }));
        });
        it('should sort transactions in descending order by creation date', async () => {
            mockTypeOrmRepository.find.mockResolvedValue([]);
            await repository.getByUser(organisationId, userId);
            expect(mockTypeOrmRepository.find).toHaveBeenCalledWith(expect.objectContaining({
    ...
    describe('getById', () => {
        ...
        it('should enforce organisation isolation', async () => {
            ...
            expect(mockTypeOrmRepository.findOne).toHaveBeenCalledWith(expect.objectContaining({
                where: {
                    organisationId: otherOrgId,
                    id: transactionId,
                },
    ...
    describe('deleteByUser', () => {
        it('should delete all transactions for a user', async () => {
            ...
            expect(mockLogger.warn).toHaveBeenCalledWith('Transactions deleted for user', expect.objectContaining({
                organisationId,
                userId,
                deletedCount: 3,
            }));
    ...
    describe('updateStatus', () => {
        it('should update transaction status', async () => {
            ...
            const result = await repository.updateStatus(organisationId, transactionId, 'completed');
            expect(result).toEqual(updatedTransaction);
            expect(mockTypeOrmRepository.save).toHaveBeenCalled();
            expect(mockLogger.info).toHaveBeenCalledWith('Transaction status updated', expect.objectContaining({
                transactionId,
                organisationId,
                newStatus: 'completed',
            }));
        ...
        it('should return null for non-existent transaction', async () => {
            mockTypeOrmRepository.findOne.mockResolvedValue(null);
            const result = await repository.updateStatus(organisationId, 'non-existent', 'completed');
            expect(result).toBeNull();
